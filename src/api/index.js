...
export function getSongDetailsApi(id) {
  return '/api/song/detail/?id=' + id + '&ids=%5B' + id + '%5D'
}

export function getPlaylistDetailsApi(id) {
  return '/api/v6/playlist/detail/?id=' + id
}

export function SearchDetailsApi(q, limit, offset) {
  return '/api/cloudsearch/pc?type=1&s=' + q + '&limit=' + limit + '&offset=' + offset
}

export function getSongLyricsApi(id) {
  return '/api/song/lyric?os=pc&id=' + id + '&lv=-1&tv=1'
}

export function getSongUrlApi(id) {
  return '/api/song/enhance/player/url?ids=[' + id + ']&br=2147483647'
}
...
const api = {
  ...
  async downloadSongFile(source, 
                         ...
                         downloadAlbum, 
                         donwloadPublishTime) {
    api.downloadFile(downloadUrl, downloadTitle + '.mp3')
